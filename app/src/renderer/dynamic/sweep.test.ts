...
const run = (style: string, skin: 'clawd' | 'kodek', scene: string, o: { saving?: boolean; reduced?: boolean } = {}, secs = 3, fps = 15) => {
  ...
  for (let f = 0; f < secs * fps; f++) {
    const r = recorder();
    p.frame(r.ctx, { dt: 1 / fps, t0: 1 + f / fps, X: 60, Y: 40, u: 0.3, animate: true, saving: !!o.saving, reduced: !!o.reduced, dpr: 1, look: { style: style as never, motion: 'dynamic' } });
    frames.push(r.log);
...
describe('dynamic sweep', () => {
  it('every dynamic scene in every style and skin: finite, balanced, capped, ≤ 3 impact frames per second', () => {
    for (const scene of Object.keys(SCENES_DYNAMIC)) for (const style of STYLE_IDS) for (const skin of ['clawd', 'kodek'] as const) {
  ...
  it('everything stays inside the 48 px taskbar (y ≥ 0) at u = 0.3', () => {
    for (const scene of Object.keys(SCENES_DYNAMIC)) for (const style of ['clean', 'sticker', 'pixel']) for (const skin of ['clawd', 'kodek'] as const) {
      ...
      for (let f = 0; f < 60; f++) {
        const e = extent();
        p.frame(e.ctx, { dt: 0.1, t0: 1 + f / 10, X: 60, Y: 40, u: 0.3, animate: true, saving: false, reduced: false, dpr: 1, look: { style: style as never, motion: 'dynamic' } });
